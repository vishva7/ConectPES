...
export default function UpdateCertificate({ params }: { params: { slug: string } }) {
    ...
    async function onSubmit(formdata: z.infer<typeof FormSchema>) {
        try {
            ...
            let response = await axios.put(
                `${process.env.NEXT_PUBLIC_BACKEND_URL}/certificates/update/${params.slug}`,
                formdata
            );
            if (response.status === 200) {
            ...
            <Form {...form}>
                ...
                >
                    <div className="space-y-4 text-center w-full max-w-[400px] lg:max-w-[800px]">
                        <div className="text-3xl font-bold">Update a Certificate</div>
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem className="space-y-2 text-left">
                                    <FormLabel>Certificate GDrive Link</FormLabel>
                                    <FormControl>
                                        <Input type="text" {...field} />
                                    </FormControl>
                                    <FormMessage />
