    ...
    const fetchHome = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/home/all`
        );
        let newHomePosts = response.data.map((home: homePost) => {
          const matchResult = home.image.match(/file\/d\/(.*?)\//);
          if (matchResult) {
            const fileId = matchResult[1];
            const newImageUrl = `https://drive.google.com/uc?export=view&id=${fileId}`;
            return { ...home, image: newImageUrl };
          } else {
            return home;
          }
        });
        setHome(newHomePosts);
        setIsLoading(false);
      } catch (error) {
        console.log(error);
        setError(true);
