...
export default function UpdateFacility({ params }: { params: { slug: string } }) {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/facilities/${params.slug}`
        );
  ...
  async function onSubmit(formdata: z.infer<typeof FormSchema>) {
    try {
      ...
      let response = await axios.put(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/facilities/update/${params.slug}`,
        formdata
      );
      if (response.status === 200) {
      ...
      <Form {...form}>
        ...
        >
          <div className="space-y-4 text-center w-full max-w-[400px] lg:max-w-[800px]">
            <div className="text-3xl font-bold">Update a Facility</div>
